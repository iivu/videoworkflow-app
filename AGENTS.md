...
- `adonisjs` has its own CLI tooling [documentation](https://docs.adonisjs.com/guides/ace/introduction); before creating any new file, first check whether there is an available command.
- The project uses `adonisjs` [Type-safe API client](https://docs.adonisjs.com/guides/frontend/api-client); after any backend change, run `pnpm --filter api dev` to auto-generate `.adonisjs`.
- `model` inherits from `scheme`, and `scheme` is auto-generated by `adonisjs` from `migration`; it is regenerated after each `migration` run, and **must not be edited manually**.
- **Do not run any database-related operations**. If needed, inform the user and continue subsequent work after the user completes those steps.

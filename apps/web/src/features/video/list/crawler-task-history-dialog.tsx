...
export function CrawlerTaskHistoryDialog({ open, onOpenChange }: Props) {
  const [page, setPage] = useState(1);
  const taskQuery = useQuery({
    ...query.videos.listCrawlerTasks.queryOptions({ query: { page, size: PAGE_SIZE } }),
    enabled: open,
  });
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[calc(100vh-2rem)] max-w-4xl! grid-rows-[auto_minmax(0,1fr)_auto]">
        <DialogHeader>
          ...
          <DialogDescription>查看已提交的视频提取任务及处理结果</DialogDescription>
        </DialogHeader>
